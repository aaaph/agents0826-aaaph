...
def ask(
    ...
    жорстким форматом відповіді в промпті, а не семплінгом."""
    resp = _call(
...
def run_agent(system: str, tools: list, query: str, on_step=None) -> dict:
    ...
    for turn in range(MAX_TURNS):
        ...
        try:
            resp = _call(**kwargs)
        except Exception as e:  # ← збій API  # noqa: BLE001
            return {
                "answer": "Сервіс тимчасово недоступний. Передаю звернення оператору.",
                "outcome": "api_error",
